    ...
    "skill":{
        ...
        "g":{
            ...
            "run": function (player, world, abilityBuffer, abilityPos){
                player.abilityCD[abilityPos] = this.stats.cooldown * tickRate
                player.moveable -= 1
                player.hidden += 1
                player.invulnerable += 1
                var newUID = uid()
                player.runningAbilities[newUID] = createNewAbility(newUID, player.ID, "g", 0.1, null)
            },
            ...
            "loop": function (player, world, ability){
                if (ability.time >= ability.duration){
                    this.clear(player, world, ability)
                } else {
                    ability.time += 1
                    dashPlayer(player, this.stats.dist, this.stats.duration)
                }   
            }
        },
        "h":{
